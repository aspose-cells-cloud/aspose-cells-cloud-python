...
class Form:
    ...
    selected_cell: Optional[LinkElement] = None
    drop_down_lines: Optional[int] = None
    item_count: Optional[int] = None
    selected_cells: Optional[List[LinkElement]] = None
    selection_type: Optional[str] = None
    is_checked: Optional[bool] = None
    current_value: Optional[int] = None
    min_: Optional[int] = None
    max_: Optional[int] = None
    incremental_change: Optional[int] = None
    page_change: Optional[int] = None
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.input_range is not None:
            result["InputRange"] = self.input_range
        if self.selected_index is not None:
            result["SelectedIndex"] = self.selected_index
        if self.selected_value is not None:
            result["SelectedValue"] = self.selected_value
        if self.selected_cell is not None:
            result["SelectedCell"] = self.selected_cell.to_dict()
        if self.drop_down_lines is not None:
            result["DropDownLines"] = self.drop_down_lines
        if self.item_count is not None:
            result["ItemCount"] = self.item_count
        if self.selected_cells is not None:
            result["SelectedCells"] = [x.to_dict() for x in self.selected_cells]
        if self.selection_type is not None:
            result["SelectionType"] = self.selection_type
        if self.is_checked is not None:
            result["IsChecked"] = self.is_checked
        if self.current_value is not None:
            result["CurrentValue"] = self.current_value
        if self.min_ is not None:
            result["Min"] = self.min_
        if self.max_ is not None:
            result["Max"] = self.max_
        if self.incremental_change is not None:
            result["IncrementalChange"] = self.incremental_change
        if self.page_change is not None:
            result["PageChange"] = self.page_change
