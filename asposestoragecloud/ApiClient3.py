...
class ApiClient3(object):
    ...
    def sign(self, url_to_sign, appSid, apiKey):
        ...
        signature = hmac.new(apiKey.encode('utf-8'), url_part_to_sign.encode('utf-8'), hashlib.sha1).digest()
        signature = base64.b64encode(signature)[:-1]
    ...
    def callAPI(self, resourcePath, method, queryParams, postData, headerParams=None, files=None):
        ...
        if mergedHeaderParams:
            for param, value in mergedHeaderParams.items():
                headers[param] = ApiClient3.sanitizeForSerialization(value)

        if self.cookie:
            headers['Cookie'] = ApiClient3.sanitizeForSerialization(self.cookie)
        ...
        if method in ['GET']:
            ...
            if postData:
                postData = ApiClient3.sanitizeForSerialization(postData)
    ...
    def sanitizeForSerialization(obj):
        ...
        if isinstance(obj, type(None)):
            ...
            return [ApiClient3.sanitizeForSerialization(subObj) for subObj in obj]
            ...
            return {key: ApiClient3.sanitizeForSerialization(val)
